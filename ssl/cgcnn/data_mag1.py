...
def CIF_Lister(material):
    n_features = material[0][0][0]
    e_features = material[0][0][1]
    e_features = e_features.view(-1, 41)
    a_matrix = material[0][0][2]
    graph_crystal = Data(x=n_features, edge_attr=e_features, edge_index=a_matrix,
...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
    ...
    train_index, test_index = train_test_split(indices, test_size=test_ratio, random_state=seed)

    if return_val:
        train_index, val_index = train_test_split(train_index, test_size=val_ratio/(val_ratio + train_ratio),
                                                  random_state=seed)
...
class CIFData(Dataset):
    ...
    def __init__(self, root_dir, max_num_nbr=12, radius=8, dmin=0, step=0.2,
        ...
        # ------------------------------------------------
        self.model = CGCNN(n_conv=5, atom_fea_len=64, drop_ratio=0, stage='get_emb').to("cpu")
        checkpoint = torch.load("cgcnn_sl_mag.pth", map_location='cpu')
    ...
    def __getitem__(self, idx):
        ...
        # ----------------- 2021/11/11 Add SL-magmom ------------------------------------
        CRYSTAL_DATA = CIF_Dataset_pg(cif_id=cif_id, root_dir=self.root_dir)
        data = CIF_Lister(material=CRYSTAL_DATA)
        self.model.eval()
        with torch.no_grad():
            mag_fea = self.model(data)
        # ----------------------------------------------------------------------------
        ...
        # ----------------------- 11/11 ------------------------------------
        atom_fea = torch.cat([atom_fea, mag_fea], 1)
        # -----------------------------------------------------------------
...
class CIF_Dataset_pg(Dataset):
    def __init__(self, cif_id=None, max_num_nbr=12, radius=8, dmin=0, step=0.2, root_dir='DATA/CIF-DATA/'):
        self.root_dir = root_dir
        # ----------------------------
        self.cif_id = cif_id
        # ----------------------------
    ...
    @functools.lru_cache(maxsize=None)  # Cache loaded structures
    def __getitem__(self, idx):
        crystal = Structure.from_file(os.path.join(self.root_dir, str(self.cif_id) + '.cif'))
        atom_fea = np.vstack([self.ari.get_atom_fea(crystal[i].specie.number) for i in range(len(crystal))])
        atom_fea = torch.Tensor(atom_fea)
        # ------------------------- 2021/4/20 -----------------------------------------
        ...
        return (atom_fea, nbr_fea, nbr_fea_idx), atom_index, self.cif_id, [crystal[i].specie for i in
                                                                           range(len(crystal))]
...
class CGCNN(nn.Module):
    ...
    def __init__(self, orig_atom_fea_len=92, nbr_fea_len=41,
        ...
        # self.neg_slope = 0.2
        self.convs = nn.ModuleList([CGConv(atom_fea_len, dim=atom_fea_len, batch_norm=False)
    ...
    def forward(self, data):
        ...
        if self.stage == 'get_emb':
            return torch.cat([atom_embedding[0], atom_embedding[1]], axis=1)
        else:
            return x

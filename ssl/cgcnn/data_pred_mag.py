...
class CIF_Lister(Dataset):
    def __init__(self, crystals_ids, full_dataset, norm_obj=None, normalization=None, df=None, src='CGCNN'):
    ...
    def __getitem__(self, idx):
        ...
        graph_crystal = Data(x=n_features, y=y, edge_attr=e_features, edge_index=a_matrix,
                             ...
                             atom_specie_number=atom_specie_number)
...
class CIF_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --------------------- 2021/11/9 Add magmom ----------------------
        magmom_list = []
        for idx in range(len(crystal)):
            magmom = np.array(list(crystal[idx].magmom))
            magmom_size = np.linalg.norm(magmom)
            magmom_list.append(magmom_size)
        magmom_tensor = torch.Tensor(magmom_list)
        # ---------------------------------------------------------------
        ...
        return (atom_fea, nbr_fea, nbr_fea_idx), target, magmom_tensor, atom_index, cif_id, \
